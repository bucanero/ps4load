...
static int SetText(SDL_Surface* surface, const char* txt, FT_Face face, Color fgColor, Color bgColor)
{
    int rc;
    int xOffset = 0;
    int yOffset = 0;
...
static SDL_Texture* CreateText(SDL_Renderer *renderer, const char* txt, FT_Face fontFace, Color fgColor, Color bgColor, int *outWidth)
{
    ...
    *outWidth = SetText(tempSurface, txt, fontFace, fgColor, bgColor);
...
void DrawString(SDL_Renderer *renderer, const char* msg, FT_Face font, Color color, int x, int y)
{
    ...
    // Center it
    pos.x = x ? x : ((1920 - pos.x) / 2);
    pos.y = y ? y : ((1080 - pos.h) / 2);

    SDL_RenderCopy(renderer, tmpText, NULL, &pos);
